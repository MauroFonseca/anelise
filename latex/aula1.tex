...
\begin{frame}{Estratificação: analogia da linha aérea}
  \begin{center}
    \begin{tikzpicture}[font=\scriptsize,
      ...
      \draw[flecha] (b) -- (b2);
      % etiquetas à direita
      \node[right=2mm of t2,align=left](e1){capa ``bilhete''};
      ...
      \node[right=2mm of r,align=left](e5){capa ``roteamento''};
    ...
    \end{tikzpicture}
  \end{center}
  Cada capa implementa um \textbf{serviço} mediante suas próprias ações internas, apoiando-se no serviço da capa imediatamente inferior.
\end{frame}
...
\begin{frame}{Pilha de protocolos da Internet}
  \begin{center}
    \begin{tikzpicture}[font=\scriptsize,
      ...
      \node[capa,below=7mm of transp] (rede) {Rede\\roteamento de datagramas origem$\rightarrow$destino\\(IP, protocolos de roteamento)};
...
\begin{frame}{Modelo de referência ISO/OSI}
  \begin{center}
    \begin{tikzpicture}[font=\scriptsize,
      ...
      \node[capa](ap){7. Aplicação};
      \node[capa,below=5mm of ap](pr){6. Apresentação\\(significado dos dados: cifragem, compressão, convenções)};
      \node[capa,below=5mm of pr](se){5. Sessão\\(sincronização, \emph{checkpointing}, recuperação)};
      \node[capa,below=5mm of se](tr){4. Transporte};
      \node[capa,below=5mm of tr](re){3. Rede};
      \node[capa,below=5mm of re](em){2. Enlace};
      \node[capa,below=5mm of em](fi){1. Física};
      \node[right=1.6cm of pr,text width=3.6cm,align=left,draw,rounded corners=1.5mm,fill=aneliseLaranja!14](obs){A pilha da Internet ``perde'' estas capas!\\Serviços como cifragem/compressão são implementados na \textbf{aplicação}.};
...
\begin{frame}{Encapsulação}
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  \textbf{Encapsulação:} cada capa agrega o seu cabeçalho ($H$); o destinatário realiza a \textbf{desencapsulação} inversa.
\end{frame}
  ...
  \begin{columns}
    \begin{column}{0.52\textwidth}
      ...
      \begin{itemize}
        ...
        \item na recepção, entrega segmentos à capa de transporte
...
% ---------------------------------------------------------------------------
\begin{frame}{Serviços da capa de enlace}
  \begin{columns}
    \begin{column}{0.5\textwidth}
...
\begin{frame}{Endereços MAC e ARP}
  \begin{itemize}
    \item \textbf{IP (32 bits / 128 bits)}: endereço de capa de rede da interface --- usado para reenvio (capa 3)
...
\begin{frame}{Leitura recomendada}
  \begin{itemize}
    ...
    \item \textbf{\tanenbaum{}} --- cap.~5 (capa de rede)
  \end{itemize}
