...
\begin{frame}{Estratificação: analogia da linha aérea}
  \begin{center}
    \begin{tikzpicture}[font=\scriptsize,
      ...
      \node[caja,above=9mm of g] (p) {pista (takeoff/landing)};
...
\begin{frame}{Subredes (subnets)}
  \begin{itemize}
    \item Endereço IP $=$ \textbf{parte de subred} (bits altos) + \textbf{parte de host} (bits baixos)
...
\begin{frame}{Protocolos de roteamento: objetivo}
  ...
  \end{block}
  \begin{itemize}
    \item \textbf{Caminho}: sequência de roteadores que os pacotes atravessarán
    \item ``Bom'': menor ``custo'', ``mais rápido'', ``menos congestionado''
    \item \alert{``Roteo é um dos top-10 desafios de rede!''}
  \end{itemize}
  \begin{exampleblock}{Escalabilidade --- o problema}
    \begin{itemize}
      ...
      \item o intercambio de tabelas de roteamento saturaría os enlaces
...
\begin{frame}{ICMP --- Internet Control Message Protocol}
  \begin{itemize}
    \item Usado por hosts e roteadores para comunicar informação de nível de rede
      ...
      \end{itemize}
    \item Mensajes ICMP viajam \textbf{dentro de datagramas IP}
  ...
  \begin{center}
    ...
    \begin{tabular}{@{}lll@{}}
      ...
      0 & 0 & echo reply (ping)\\
      3 & 0/1/2/3 & destino red/host/protocolo/porta inalcançável\\
      3 & 6/7 & red/host desconhecido\\
...
\begin{frame}{Traceroute e ICMP: como funciona}
  \begin{columns}
    \begin{column}{0.55\textwidth}
      ...
      \begin{itemize}
        \item o segmento UDP chega ao destino; o destino devólve \textbf{ICMP tipo 3 código 3} (porta inalcançável)
      \end{itemize}
    \end{column}
...
% ---------------------------------------------------------------------------
\begin{frame}{Análisis de pacotes com Wireshark}
  \begin{columns}
    \begin{column}{0.55\textwidth}
      ...
      \begin{enumerate}
        ...
        \item Analice a resposta
        \item Instale Wireshark (\url{https://www.wireshark.org})
        \item Repita o traceroute e analice os pacotes ICMP em todas as capas
      \end{enumerate}
    \end{column}
    \begin{column}{0.45\textwidth}
      \begin{center}
      \begin{tikzpicture}[font=\scriptsize,
        ...
        \node[right=0mm of ana.south east,font=\tiny,text width=3cm,anchor=north west](nota){captura requer privilegios de root};
      \end{tikzpicture}
      \end{center}
    \end{column}
  \end{columns}
...
\begin{frame}{Referências e bibliografia}
  ...
  \begin{exampleblock}{Renovação do exercício}
    Além do traceroute: levante um servidor HTTP local (\texttt{python3 -m http.server}) e analice com Wireshark o handshake TCP y HTTP/1.1$\rightarrow$HTTP/2 (ALPN).
  \end{exampleblock}
\end{frame}
